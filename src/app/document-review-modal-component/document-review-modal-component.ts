  ...
  template: `
    <div class="modal-backdrop">
      <div class="modal-panel">
        <h3>Review Document: {{ document.name }}</h3>
        <p><strong>Filename: </strong>{{ document.filename }}</p>
        <p><a [href]="document.url" target="_blank">View Document</a></p>
...
export class DocumentReviewModalComponent {
  ...
  submit() {
    ...
    this.service.reviewEmployeeDocument(
      this.document.id,
      this.approve,
      this.approve ? undefined : this.rejectionReason
    ).subscribe({
      next: () => {
      ...
      error: (err) => {
        this.errorMessage = 'Failed to submit review: ' + (err.message || 'Unknown error');
        this.submitting = false;
      }
    });
  }
